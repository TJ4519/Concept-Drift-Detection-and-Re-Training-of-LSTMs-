...
Below is a simple Python simulation that demonstrates the core idea of using the Wasserstein distance on reconstruction error distributions that drift over time.
...
# Simulate a baseline reconstruction error distribution (normal operating conditions)
np.random.seed(42)
baseline_errors = np.random.normal(loc=0.5, scale=0.1, size=1000)

# Function to simulate drifting reconstruction errors (e.g., due to soot buildup)
def simulate_drift(baseline, drift_factor, size=1000):
    # Drift is modeled as an increase in mean error
    drifted_errors = np.random.normal(loc=baseline.mean() * (1 + drift_factor), 
                                      scale=baseline.std(), size=size)
    return drifted_errors

# Simulate a series of batches with increasing drift
drift_factors = np.linspace(0, 0.5, 10)  # 0% to 50% drift
wasserstein_distances = []

for drift in drift_factors:
    current_errors = simulate_drift(baseline_errors, drift)
    wd = wasserstein_distance(baseline_errors, current_errors)
    wasserstein_distances.append(wd)
    print(f"Drift factor: {drift:.2f} - Wasserstein Distance: {wd:.3f}")

# Plot the Wasserstein distance as drift increases
plt.figure(figsize=(8, 5))
plt.plot(drift_factors, wasserstein_distances, marker='o')
plt.title('Wasserstein Distance vs. Drift Factor')
plt.xlabel('Drift Factor (Increase in Mean Error)')
plt.ylabel('Wasserstein Distance')
plt.grid(True)
plt.savefig('wasserstein_drift.png')
plt.show()
